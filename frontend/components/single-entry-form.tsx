...
export function SingleEntryForm({ onSubmit, isLoading, initialData }: SingleEntryFormProps) {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (!courierType) throw new Error("快递类型不存在")
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-medium">单条发货数据录入</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input type="number" min="1" step="1" placeholder="输入发货数量" {...field} />
                        </FormControl>
                        <FormDescription>请输入大于0的整数</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
