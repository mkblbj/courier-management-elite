...
export function EnvSwitcher() {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      logInfo("当前环境配置", { env, isDev, debug })
    }
  }, [env, isDev, debug])
  ...
  const handleEnvironmentChange = (newEnv: Environment) => {
    ...
    logInfo("环境已切换", { from: env, to: newEnv })
    toast({
      title: "环境已切换",
      description: `当前环境: ${newEnv}`,
      variant: "success",
    })
  }
        ...
        <div className="fixed bottom-16 right-4 z-50 w-80 animate-fade-in">
          <Card className="border shadow-lg">
            <CardHeader className="pb-3">
              ...
              <CardDescription>切换环境配置以测试不同环境</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Label htmlFor="api-url" className="cursor-pointer">
                    API 地址
                  </Label>
                </div>
