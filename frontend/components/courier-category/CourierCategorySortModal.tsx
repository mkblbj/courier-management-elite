...
interface CourierCategorySortModalProps {
      open: boolean;
...
export function CourierCategorySortModal({ open, categories, onClose, onSort, dialogDescription }: CourierCategorySortModalProps) {
      const [items, setItems] = useState<CourierCategory[]>([]);
      ...
      return (
            <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
                  <DialogContent className="sm:max-w-[500px]">
                        <DialogHeader>
                              <DialogTitle>{t('courier:adjust_category_sort')}</DialogTitle>
                              {dialogDescription && (
                        ...
                        <DialogFooter>
                              ...
                              <Button onClick={handleSubmit} disabled={isSubmitting || items.length === 0}>
                                    {isSubmitting ? t('courier:saving') : t('courier:save_sort')}
                              </Button>
                        </DialogFooter>
                  </DialogContent>
            </Dialog>
