...
```
创建一个店铺管理页面，使用React、Next.js、Tailwind CSS和shadcn/ui组件。该页面应该有顶部标签导航，包含"快递类型"和"店铺管理"两个标签，默认选中"店铺管理"。店铺管理下有两个子标签："店铺类别"和"店铺管理"，默认选中"店铺管理"。

页面主体是按类别分组展示的店铺数据表格。每个类别有一个标题行显示类别名称和该类别下的店铺数量（例如"[淘宝] 共2家店铺"），标题行下方是该类别的店铺表格。表格列包含：店铺名称、状态（使用开关组件显示启用/禁用状态）、排序（显示数字）和操作按钮组（编辑、删除）。

表格上方有添加新店铺按钮和搜索栏，搜索功能应作用于所有类别的店铺。类别之间有明显的视觉分隔，使用不同的背景色或边框。每个类别的表格都有独立的标题行，表格设计保持一致。

所有表格使用淡色背景和圆角边框设计，保持现代简约风格。表格行在悬停时有背景色变化效果。页面采用响应式设计，在移动设备上自动调整布局。
```
...
1. 店铺类别与店铺管理页面：
   ...
   - 店铺管理下的子标签（店铺类别/店铺管理）
   - 店铺类别列表（列：类别名称、排序、操作）
   - 按类别分组的店铺数据表格（每个类别独立表格，列：店铺名称、状态、排序、操作）
   - 添加/编辑类别对话框
   - 添加/编辑店铺对话框（包含类别选择）
   - 类别排序功能对话框
   - 店铺排序功能对话框（按类别分组）
