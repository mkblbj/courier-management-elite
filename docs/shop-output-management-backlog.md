...
**描述**: 创建所需的数据库表，包括 shops 表和 shop_outputs 表，并设计必要的索引和外键约束。

**验收标准**:

- 创建 shops 表，包含 id、name、is_active、sort_order、remark、created_at、updated_at 字段
...
- 在现有前端项目中添加 shop-outputs 相关路由
- 创建必要的基础组件目录结构
- 配置与服务器 API 通信的基础设置

## Epic 1: 店铺管理功能

该 Epic 包含对店铺信息进行管理的所有功能，是系统的基础部分。
...
- 在/courier-types 页面添加标签切换功能，支持在快递类型和店铺管理之间切换
- 店铺列表展示店铺名称、状态和排序信息
- 支持按名称或创建时间排序
- 支持搜索功能
...
**描述**: 实现统计维度选择功能，支持按店铺、按快递类型、按日期进行统计。
...
2. Epic 1（店铺管理功能）必须在 Epic 2（出力数据管理功能）之前完成，因为出力数据需要关联到店铺
3. Epic 2（出力数据管理功能）必须在 Epic 3（统计分析功能）和 Epic 4（仪表盘集成）之前完成，因为这些功能依赖于出力数据
...
1. 完成 Epic 0 的所有故事
2. 完成 Epic 1 的所有故事
3. 完成 Epic 2 的故事 2.1 到 2.7
4. 完成 Epic 4 的故事 4.1 和 4.3
5. 完成 Epic 2 的故事 2.8
6. 完成 Epic 3 的所有故事
7. 完成 Epic 4 的剩余故事

这个顺序确保了最核心的功能（店铺管理和出力数据管理）优先实现，同时也让部分高价值的仪表盘功能能够较早地交付给用户。
