...
- 故事 0.1: 数据库迁移与设计：创建 shops 和 shop_outputs 表，设计索引和外键约束
- 故事 0.2: API 路由与控制器设置：配置必要的 API 路由和控制器框架
- 故事 0.3: 前端路由与基础组件设置：配置前端路由和基础组件

### Epic 1: 店铺管理功能

- 故事 1.1: 店铺列表展示：在现有页面添加店铺管理标签页，实现列表展示
- 故事 1.2: 店铺添加功能：实现添加新店铺的功能
- 故事 1.3: 店铺编辑功能：实现编辑现有店铺信息的功能
- 故事 1.4: 店铺删除功能：实现删除店铺的功能
- 故事 1.5: 店铺状态切换功能：实现启用/禁用店铺的功能
- 故事 1.6: 店铺排序功能：实现调整店铺排序顺序的功能

### Epic 2: 出力数据管理功能

- 故事 2.1: 出力数据页面基础 UI：创建出力数据管理页面，实现基本 UI 布局
- 故事 2.2: 日期与店铺选择组件：实现日期选择器和店铺选择组件
- 故事 2.3: 出力数据录入表单：实现出力数据录入表单及其验证逻辑
- 故事 2.4: 最近录入数据展示：实现最近录入的出力数据展示功能
- 故事 2.5: 出力数据编辑功能：实现编辑已录入出力数据的功能
- 故事 2.6: 出力数据删除功能：实现删除出力数据记录的功能
- 故事 2.7: 当日数据汇总展示：实现当日出力数据的汇总展示功能
- 故事 2.8: 高级筛选面板：实现高级筛选功能，支持多条件组合查询

### Epic 3: 统计分析功能

- 故事 3.1: 统计分析页面基础 UI：在现有页面添加出力统计选项卡
- 故事 3.2: 统计维度选择功能：实现按店铺、按快递类型、按日期进行统计
- 故事 3.3: 按店铺统计功能：实现按店铺统计出力数据的功能
- 故事 3.4: 按快递类型统计功能：实现按快递类型统计出力数据的功能
- 故事 3.5: 按日期统计功能：实现按日期统计出力数据的功能
- 故事 3.6: 数据导出功能：实现统计数据导出功能

### Epic 4: 仪表盘集成

- 故事 4.1: 今日出力卡片：在仪表盘添加今日店铺出力情况卡片
- 故事 4.2: 明日出力预测卡片：在仪表盘添加明日店铺出力预测卡片
- 故事 4.3: 仪表盘数据 API 集成：实现仪表盘所需的数据 API
- 故事 4.4: 仪表盘出力趋势图：添加出力数据趋势图，展示近期变化情况
